...
export default function VendorsView({ vendors: initialVendors, token }: Props) {
  ...
  const applyFilters = async (currentPage: number, currentLimit: number, filters: { [key: string]: string }) => {
    try {
      const response = await Vendor.findAll(token, true, currentLimit, currentPage, filters);
      const data = response?.data?.data?.vendors || [];
      setVendors(data);
      setTotalPages(response?.data?.data?.total_pages || 1);
    } catch (error: any) {
      notifyError(`Lỗi khi lấy dữ liệu: ${error?.message || "Không thể kết nối với server"}`);
    }
  };
  useEffect(() => {
                          ...
                          {headCell.id !== "action" ? (
                              headCell.id === "status" ? (
                                  <Select
                                      ...
                                      onChange={(e) => {
                                        handleSearchChange(headCell.id, e.target.value as string);
                                      }}
                                      displayEmpty
                                  >
                                    <MenuItem value="">
