...
export default function CategoriesView({ categories: initialCats, token }: Props) {
    ...
    const handleDelete = async () => {
        if (selectedCategoryId) {
            try {
                const response = await Categories.delete(token, true, selectedCategoryId);
                ...
                if (response.data.success) {
                    ...
                    notifyError(
                        "Xóa danh mục thất bại: " + (response.data.message || "Vui lòng thử lại.")
                    );
                }
            } catch (error) {
    ...
    const applyFilters = async (page: number, limit: number, filters = searchValues) => {
        try {
            const response = await Categories.findAll(token, true, limit, page, filters);
