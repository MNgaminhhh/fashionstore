...
export default function SubCategoryView({ subcategories: initialSubs, token }: Props) {
    const [subcategories, setSubcategories] = useState(initialSubs?.sub_categories || []);
        ...
        if (selectedSubCategoryId) {
            try {
                ...
                if (response.success) {
                    ...
                    notifyError(
                        "Xóa danh mục con thất bại: " + (response.message || "Vui lòng thử lại.")
                    );
                }
            } catch (error) {
    ...
    const applyFilters = async (page: number, limit: number, filters = searchValues) => {
        try {
            const response = await SubCategory.findAll(token, true, limit, page, filters);
