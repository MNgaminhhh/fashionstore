...
import { StyledPagination, StyledTableCell } from "../../../../../../components/table/styles";
...
export default function ChildCategoryView({
    ...
    const [childCategories, setChildCategories] = useState<ChildCategoryModel[]>(initialChildCategories?.child_categories || []);
    const [totalPages, setTotalPages] = useState<number>(initialChildCategories?.total_pages || 1);
    ...
    const handleDelete = async () => {
        if (selectedChildCategoryId) {
            try {
                const response = await ChildCategory.delete(token, true, selectedChildCategoryId);
                if (response.success) {
                    notifySuccess("Danh mục con đã được xóa thành công.");
                    await applyFilters(currentPage, currentLimit);
                } else {
                    notifyError("Xóa danh mục con thất bại: " + (response.message || "Vui lòng thử lại."));
                }
            } catch (error) {
    ...
    const applyFilters = async (page: number, limit: number, filters = searchValues) => {
        try {
            const response = await ChildCategory.findAll(token, true, limit, page, filters);
    ...
    const handleToggleStatus = async (id: string, currentStatus: string) => {
        try {
            ...
            if (response.success) {
                ...
                notifyError("Cập nhật trạng thái thất bại: " + (response.message || ""));
            }
        } catch (error) {
    ...
    return (
        <WrapperPage title="Quản Lý Danh Mục Con Cấp 2">
            <Box display="flex" justifyContent="flex-end" mb={2}>
                <Button
                    href="/admin/categories/sub-category/child/create"
                    color="primary"
                    variant="contained"
                    startIcon={<Add />}
                    LinkComponent={Link}
                                            ...
                                            {childCategoryTableHeading.map((headCell) => (
                                                ...
                                                >
                                                    ...
                                                    {headCell.id === "status" ? (
                                                        ...
                                                        >
                                                            <MenuItem value="-1">Vui lòng chọn giá trị</MenuItem>
